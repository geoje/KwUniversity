...
struct Box : public Shape {
	Box(Point xy, int width, int height, int arc_radius);

	void draw_lines() const;

	int height() const { return h; }
	int width() const { return w; }
	int arc_radius() const { return r; }
private:
	int w, h, r;
};

struct Regular_polygon : public Shape {
	Regular_polygon(Point xy, int number, int distance);
...
struct Star : public Shape {
	Star(Point xy, int number, int distance1, int distance2);

	void draw_lines() const;

	int number() const { return n; }
	int distance1() const { return r1; }
	int distance2() const { return r2; }
private:
	int n, r1, r2;
};
